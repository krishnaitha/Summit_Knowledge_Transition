  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
        <Card className="bg-hero-grid relative overflow-hidden border-0 text-white shadow-lg shadow-slate-900/10">
            ...
            <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm text-slate-300">
                  One place to finish docs, ask AI questions, and complete assessments without
              ...
              {[
                ...
                >
                  <div className="flex items-center justify-between">
                    <p className="text-[11px] font-medium tracking-wider text-slate-200 uppercase">
                      {label}
                    </p>
                    <span
        ...
        <Card className="glass-panel-solid">
          ...
          <CardContent className="space-y-3">
            {nextActionProject ? (
              ...
              <p className="text-sm text-slate-500">No product assigned yet.</p>
            )}
            ...
            {adminProjectIds.length > 0 && (
              <div className="rounded-xl border border-blue-200 bg-blue-50/60 p-3">
                ...
                <p className="text-sm text-slate-700">
                  You manage {adminProjectIds.length} product{adminProjectIds.length > 1 ? 's' : ''}
                  .
                </p>
              </div>
            )}
        ...
        <div className="space-y-6">
          <section className="space-y-4">
            <div className="flex flex-wrap items-end justify-between gap-3">
              <div>
                <h2 className="text-xl font-semibold text-slate-950">Assigned products</h2>
                <p className="text-sm text-slate-500">
                  Your active work queue with docs, AI, and quiz status.
                </p>
              </div>
              ...
              {projects.length ? (
                ...
                <Card>
                  <CardContent className="p-6 text-sm text-slate-500">
                    No active products assigned yet.
                  </CardContent>
                </Card>
              )}
            </div>
          ...
          {adminProjectIds.length > 0 && (
            <section className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base font-semibold text-slate-900">Managed products</h2>
                <Badge variant="info">Product Admin</Badge>
              </div>
                ...
                {stats.recentAnnouncements.map((item, i) => (
                  <Card key={i} className="border-brand-100 bg-brand-50/40">
                    <CardContent className="p-4">
                      <p className="text-brand-700 text-[11px] font-medium tracking-wider uppercase">
                        {item.projectName}
                      </p>
                  ...
                  <Card key={i} className="border-amber-100">
                    <CardContent className="p-4">
                      <div className="mb-2 flex items-center justify-between gap-3">
                        <span className="truncate rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-semibold tracking-wider text-amber-700 uppercase">
                          {bm.projectName}
                        </span>
